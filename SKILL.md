...
- **网关分级与 CCP 弹性分级映射**：
  - **【L0 即时问答】**：无卡直接作答，0 状态机开销；
  - **【L1 轻量任务】**：映射为 **CCP Tier-3**（Fast-Path 快速放行，物理免独立报告，凭据直接回写看板）；
  - **【L2 标准研发】**：根据规模与契约自动匹配 **CCP Tier-2**（局部优化，工时 ≤ 4.0h，免物理报告）或 **CCP Tier-1**（核心架构与接口契约强约束）。
