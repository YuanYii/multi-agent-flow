...
- 🎯 **单一任务原则 (SRP) 网关前置强门禁**：凡属于 L1/L2 业务需求，网关强制执行单一职责审查。一张任务卡必须且仅能由一位专属专家负责，产出单一形态的原子交付物（工时 ≤ 8.0h）。若需求涉及跨正交领域（如既有后端接口又有前端页面、既有架构设计又有业务编码），**网关强制直接触发 PM 严经理按原子工作包拆解建卡，严禁单卡承接复合全栈需求**，从源头杜绝状态混杂。
- 📝 **建卡核心要点必填备注**：创建任何任务卡（L1/L2）时，必须通过 `--remarks "<核心要点>"` 将任务的核心目标、设计要点、输入输出契约或排查要点（2~4 句话精炼概括）显式写入备注中，看板卡片与详情一目了然，严禁空白备注建卡。
