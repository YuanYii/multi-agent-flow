# 03 - 防错闭环与五层防错门控机制 (§九)

> **全规约导航**：[01 主索引](01-AI-Team-Workflow-Index.md) | [02 状态推导](02-State-Flow-Rules.md) | [04 Git规范](04-Git-Workflow-Spec.md) | [05 文档管理](05-Document-Management-Spec.md) | [06 交接协议](06-Inter-Agent-Handover-Protocol.md)

---

> **本文件说明**：规范 AI Agent 在协同研发中的越权防护、提权授权与指令拆分分派 SOP。

---

## 一、防错闭环四大链路

为防止 AI Agent 产生“机械推演”、“越权操作”或“指令乱发”，所有状态流转操作执行前必须经过五层校验：

```text
[用户指令/事件触发] 
       │
       ▼
 1. 任务类型路径推导 (A-G 类独立链推导)
       │
       ▼
 2. 角色权限拦截 (校验当前角色 ∈ 合法操作人集合)
       │
       ▼
 3. 提权授权协议 (判断是否有用户显式代行授权)
       │
       ▼
 4. 指令拆分分派 (无授权时按权限拆分并交由相应角色流转)
```

---

## 二、角色权限拦截矩阵

| 状态变更动作 | 合法操作人集合 (Allowed Operators) | 变更后处理人 (Assignee Target) | 关联场景 |
|-------------|-----------------------------------|------------------------------|---------|
| `待开始` → `进行中` | 所有执行角色 (`PM`, `DEV`, `FRONTEND`, `ARCHITECT`, `REVIEWER`, `QA`, `DOCS`, `DEVOPS`) | 自身或被分配者 | A~G 类各类任务自领取启动 |
| `已退回` → `进行中` | 所有执行角色 (`DEV`, `FRONTEND`, `ARCHITECT`, `REVIEWER`, `QA`, `DOCS`, `DEVOPS`, `PM`) | 原负责人 | 返工修复领单开工 |
| `待开始` → `已验收` | `PM` (直接确认完成) | `PM` (负责人保持为 User) | E 类用户自执行任务快捷验收 |
| `进行中` → `已验收` | `PM` (直接确认完成) | `PM` (负责人保持为 User) | E 类用户自执行任务快捷验收 |
| `进行中` → `审查中` | `DEV` / `FRONTEND` (代码/前端开发完成) / `ARCHITECT` (架构完成) | `REVIEWER` (代码/前端) 或 `PM` (架构) | A 类开发/前端、B 类架构提审 |
| `进行中` → `已完成` | 各执行角色 (`ARCHITECT`, `DEV` 环境, `FRONTEND` 环境, `DOCS`, `DEVOPS`, `REVIEWER` 专项, `QA` 专项) | `PM` | B/C/D/G 类及专项审查/测试提交验收 |
| `审查中` → `测试中` | `REVIEWER`, `PM` (审查通过) | `QA` | A 类代码/前端审查通过 |
| `审查中` → `已退回` | `REVIEWER`, `PM` (审查不通过) | 原负责人 (DEV / FRONTEND / ARCHITECT) | A/B 类审查打回 |
| `测试中` → `已完成` | `QA` (测试通过，必带 `结束时间`) | `PM` | A 类测试通过提交 PM 验收 |
| `测试中` → `已退回` | `QA` (测试不通过) | 原负责人 (DEV / FRONTEND) | A 类测试打回 |
| `已完成` → `已验收` | `PM` (最终验收通过) | `PM` (保持不变) | A/B/C/D/F/G 类终态验收 |
| `任意状态` → `已阻塞` | 所有 8 个角色 (PM, ARCHITECT, DEV, FRONTEND, REVIEWER, QA, DOCS, DEVOPS) | 保持不变 | 遭遇依赖未就绪或环境卡顿 |
| `已阻塞` → `进行中` | 所有 8 个角色 (PM, ARCHITECT, DEV, FRONTEND, REVIEWER, QA, DOCS, DEVOPS) | 原处理人 | 阻塞解除恢复执行 |

---

## 三、角色提权与临时授权协议（代行协议）

当 Agent 收到超越自身角色权限的指令（例如，Agent 身份为开发，但用户指令要求“帮我把 T0001 验收掉”）时：

1. **强行拦截机制**：
   - 检查：当前 Agent 角色 ∉ `PM`（验收合法操作人）。
   - 如果用户**未显式授权代行**，系统**拒绝直接执行该改动**。

2. **显式提权授权协议**：
   - 若用户明确指定：“以项目经理身份进行验收” 或 “授权你代行 PM 验收”。
   - Agent 必须进行**声明与留痕**：
     > [NOTICE] "检测到跨角色指令。按规范该操作需【项目经理】执行。根据您的明确授权，由我临时代行该操作，执行 [已完成 → 已验收] 流转..."
   - **CLI 强制门控**（代码层硬拦截，无声明即拒绝）：
     - 调用 `transition_task.py` 时必须显式传 `--delegated-by <来源角色>` 与 `--delegation-reason "<理由>"`；
     - `--delegated-by` 取值必须在 `_lib/core/validate_transition.py:DELEGATION_ALLOW_MATRIX` 白名单内，否则代码层 Fail-Closed 拒绝（典型合法组合：`--role PM --delegated-by DEV`、`--role QA --delegated-by PM`、`--role PM --delegated-by USER`）；
     - 跨级别互代行（如 `DEV` 代行 `FRONTEND`）允许；任意角色代行 `PM` 允许（PM 是通用收口角色）；`USER` 表示"人类用户授权代行"，优先级最高。
   - **留痕与限制**：
     - 在看板「过程描述」或日志中注明：`[代行记录] Agent代行PM执行验收 | 授权依据: 用户显式指令 | 授权时间: YYYY-MM-DD`；
     - 审计日志（`logs/audit_trail.log`）自动追加 `delegated_by` 与 `delegation_reason` 字段，供事后追溯；
     - 看板「处理人」仍设为合法目标角色（`PM`）；
     - 授权**仅对本次操作单次生效**，不改变后续角色界限。

---

## 四、指令拆分分派 SOP

在未获得提权授权的情况下，Agent 必须将复合指令拆分，仅执行自身权限内的部分，越权部分输出结构化转交清单：

### 结构化转交输出格式
```markdown
### [WARN] 权限拦截与任务转交清单

以下任务超越当前角色权限，已自动生成转交记录，需对应角色处理：

| 任务编号 | 当前状态 | 拟执行动作 | 所需角色 | 建议后续操作 |
|---------|---------|-----------|---------|-------------|
| T0012   | 已完成   | 阶段验收   | PM      | 请 PM 检查交付物后改【已验收】 |
```

---

## 五、时序真实性与防冲卡机制 (Timing Integrity & Anti-Skew)

为保障看板交付周期 (Lead Time) 与效能度量的真实可信，严格执行以下时序门控：
1. **建卡开工时间解耦**：创建任务卡【待开始】时，`start_date` 严格置空；仅当任务首次推进至【进行中】（开工时刻）时，系统才动态落盘真实的 `start_date`。
2. **打回重领开工时间保护**：当任务经历审查/测试打回（【已退回】）后重新领回【进行中】时，保留原有初始 `start_date`，确保从首次开工到终态验收的真实生命周期闭环计算。
3. **严禁秒级冲卡**：禁止在同一秒或同一批处理中无时间差连续推进 4~5 个流转节点（如待开始->进行中->审查中->测试中->已完成->已验收）；各节点流转必须伴随真实的专家交互、代码审查报告或测试执行产出。

---

## 六、终态不可逆流与打回不拆单 (Terminal Status Immutability & Defect Binding)

1. **终态锁定不可逆**：已进入【已验收】或【已取消】终态的任务，在状态机与适配器物理层均锁定为只读终态，禁止任何逆向流转为中间状态（如推回【已阻塞】或【进行中】）。
2. **打回不拆单铁律**：审查或测试不通过时，必须在原工单上改状态为【已退回】并追加 `DEF-TXXX-N` 缺陷结构化说明退回原责任人，严禁以“已取消 + 新建任务”逃避打回审计。
