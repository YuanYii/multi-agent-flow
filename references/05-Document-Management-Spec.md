...
├── 01-项目管理/                               # 轻量项目管理核心工件
│   ├── 01-需求/                               # [子目录编号] 需求规格与确认记录
│   │   ├── 01-需求规格说明书.md                # [文件编号]
│   │   └── 02-需求确认记录.md
│   ├── 02-状态报告/                           # [子目录编号] 周期报告（周报/阶段总结）
│   ├── 01-项目章程.md                         # [文件编号] 目标、范围、授权
│   ├── 02-项目计划.md                         # [文件编号] 整合计划：范围+里程碑+成本
│   ├── 03-风险与问题登记册.md                  # [文件编号] 风险+问题合并一张表
│   ├── 04-变更日志.md                         # [文件编号] 所有变更记录
│   └── 05-经验教训.md                         # [文件编号] 复盘结论，持续追加
├── 02-架构设计/                               # 系统级架构与技术方案
│   ├── 原项目文档/                            # 原工程历史架构/设计文档归档区
│   ├── 01-系统总体架构设计.md
│   ├── 02-技术选型与可行性分析.md
│   └── 03-数据结构与接口规约.md
├── 03-业务模块/                               # 业务模块目录（按功能内聚）
│   ├── 原项目文档/                            # 原工程历史模块设计归档区
│   └── {module-A}/                           # 业务模块 A（例如：log-parser）
│       ├── README.md                         # 模块功能概述
│       ├── 01-详细设计规格.md                 # 模块详细设计规格
│       ├── 02-前端交互规格/                   # [子目录编号] UI/组件/微交互产出
│       │   ├── 01-组件清单契约.md
│       │   └── 02-UX交互规格.md
│       └── 03-排查与踩坑记录.md               # 模块专属踩坑与排查记录
├── 04-研发过程/                               # 研发过程管理与交付产出
│   ├── 原项目文档/                            # 原工程历史任务/部署/报告归档区
│   ├── 01-任务/                              # [子目录编号] 任务拆解与工作包 (Work Packages)
│   ├── 02-报告/                              # [子目录编号] 阶段测试/代码审查/质量总结报告
│   │   ├── dev/                              # 开发任务报告
│   │   ├── review/                           # 代码审查报告
│   │   ├── qa/                               # 测试报告
│   │   ├── summary/                          # 阶段总结报告
│   │   └── problems/                         # 问题与排查记录
│   └── 03-操作手册/                           # [子目录编号] 平台部署与操作手册
│       ├── 01-部署手册.md
│       └── 02-用户操作指南.md
├── 05-规范标准/                               # 规范体系（工程、流程、协作）
...
轻量化三原则：
1. **一个工件一个家**：每类信息只放一个位置，砍掉交叉引用；
2. **合并同类**：计划合一份、风险+问题合一张表、复盘持续追加一个文件；
3. **复用现有系统**：看板承担任务追踪（WBS/进度可视），文档只记决策和结论，不重复记状态。
...
| 触发场景 | 更新责任人 | 更新要求 |
| :--- | :--- | :--- |
| **整体架构/选型变更** | 架构师 (ARCHITECT) | 24 小时内更新 `02-架构设计/` 下对应文档，同步修改相关 Frontmatter 版本。 |
| **模块内部设计变更** | 开发工程师 (DEV) | 同步更新 `03-业务模块/{module-name}/01-详细设计规格.md`。 |
| **前端 UI/组件变更** | 前端工程师 (FRONTEND) | 同步更新 `03-业务模块/{module-name}/02-前端交互规格/01-组件清单契约.md` 与 `02-UX交互规格.md`。 |
| **阶段/里程碑完成** | 运维/质量/PM | 在 `04-研发过程/02-报告/` 集中输出阶段报告，清理 `草稿箱/` 中的临时草稿。 |
