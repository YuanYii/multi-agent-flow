...
2. **统一语义命名**：废除 `A01`, `B01` 等抽象代号，采用英文/中文语义化目录。
### 3. 临时文档与隔离机制
* 禁止在项目根目录或 `docs/` 顶层直接散落临时笔记或测试导出的 markdown 文件。
* 未定稿草稿与中间过程调试产出统一放置在 `.drafts/` 目录。
* 项目根目录的 `.gitignore` 必须显式忽略 `.drafts/`。
...
├── 01-architecture/                   # 系统级架构与技术方案
│   ├── 原项目文档/                     # 原工程历史架构/设计文档归档区
│   ├── 01-系统总体架构设计.md
│   ├── 02-技术选型与可行性分析.md
│   └── 03-数据结构与接口规约.md
├── 02-modules/                        # 业务模块目录（按功能内聚）
│   ├── 原项目文档/                     # 原工程历史模块设计归档区
...
│   ├── tasks/                         # 任务拆解与工作包 (Work Packages)
│   ├── reports/                       # 阶段测试/代码审查/质量总结报告
│   └── guides/                        # 平台部署与操作手册
├── 04-standards/                      # 规范体系（工程、流程、协作）
│   └── 原项目文档/                     # 原工程历史编码/流程规范归档区
├── 05-templates/                      # 各类文档标准化模板
└── .drafts/                           # 临时文档与草稿中转区（写入 .gitignore）
```
