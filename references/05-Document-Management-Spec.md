...
│   │   └── 需求确认记录.md
│   ├── 项目章程.md                     # 目标、范围、授权
│   ├── 项目计划.md                     # 整合计划：范围+里程碑+成本
│   ├── 风险与问题登记册.md             # 风险+问题合并一张表
│   ├── 变更日志.md                     # 所有变更记录
│   ├── 状态报告/                       # 周期报告（周报/阶段总结）
│   └── 经验教训.md                     # 复盘结论，持续追加
├── 02-架构设计/                        # 系统级架构与技术方案
...
| **模块内部设计变更** | 开发工程师 (DEV) | 同步更新 `03-业务模块/{module-name}/design.md`。 |
| **前端 UI/组件变更** | 前端工程师 (FRONTEND) | 同步更新 `03-业务模块/{module-name}/frontend/components.md` 与 `ux-spec.md`。 |
| **阶段/里程碑完成** | 运维/质量/PM | 在 `04-研发过程/报告/` 集中输出阶段报告，清理 `草稿箱/` 中的临时草稿。 |
