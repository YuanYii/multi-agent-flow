...
- **💻 模式 1：CLI 终端 Agent** *(Claude Code / Codex / OpenCode / ZCode 等)*  
  ⚠️ **重启一次 CLI 进程**完成专家拓扑挂载，随后支持斜杠指令（如 `/yy-flow help`）或自然语言直接对话推进。
- **🖥️ 模式 2：桌面端 IDE Agent** *(Cursor / Windsurf / AutoClaw / WorkBuddy 等)*  
...
| **生效方式** | ⚠️ 首次安装需**重启一次 CLI 进程**加载专家拓扑 | ✅ **免重启**，工作区热重载直接生效 |
