...
4. **完工后流转闭环硬门禁**：任何代码/文档/审查/测试产出完成后，最后一步必须执行流转 CLI 推进状态（A 类推至【审查中】，B/C/D/G 类推至【已完成】并记录 `end_time`），否则视为未交付。
5. **防重复建单**：建卡前若看板最近 N 条任务存在名称重复，命令将终止并输出 `DUPLICATE_TASK` 候选；需用户确认后以 `--force` 重跑。
6. **任务卡必经【待开始】**：不允许直接创建为【进行中】或更高状态，必须经历生命周期完整流转。
