...
def generate_unified_proposal():
    ...
    def clear_and_add_runs(p, runs_spec, space_before=4, space_after=4, line_spacing=1.25, align=WD_ALIGN_PARAGRAPH.LEFT):
        clear_paragraph(p)
        ...
        for text, size_pt, bold, color in runs_spec:
            r = p.add_run(text)
            format_run(r, font_size_pt=size_pt, bold=bold, color=color)
    ...
    for p in doc.paragraphs:
        ...
        if "千问办公专家套件方案书" in txt:
            clear_and_add_runs(p, [("千问办公专家套件方案书", 18, True, COLOR_PRIMARY)], space_before=12, space_after=8, align=WD_ALIGN_PARAGRAPH.CENTER)
        elif "专家名称：" in txt:
            clear_and_add_runs(p, [("专家名称：", 11, True, COLOR_TEXT), ("多专家协同研发工作流 (multi-agent-flow)", 11, False, COLOR_TEXT)], space_before=4, space_after=2)
        elif "申报主体：" in txt:
            clear_and_add_runs(p, [("申报主体：", 11, True, COLOR_TEXT), ("任可", 11, False, COLOR_TEXT)], space_before=2, space_after=10)
        elif txt.startswith("一、") or txt.startswith("二、") or txt.startswith("三、") or txt.startswith("四、"):
            clear_and_add_runs(p, [(txt, 13, True, COLOR_PRIMARY)], space_before=14, space_after=6)
        elif txt.startswith("2.1") or txt.startswith("2.2") or txt.startswith("2.3") or txt.startswith("4.1") or txt.startswith("4.2"):
            clear_and_add_runs(p, [(txt, 11.5, True, COLOR_TEXT)], space_before=10, space_after=4)
        elif "一句话定位：" in txt:
            clear_and_add_runs(p, [
                ("【一句话定位】 ", 10.5, True, COLOR_PRIMARY),
                ("面向敏捷研发团队与独立开发者的 8 位 AI 专家协同工作流套件，提供契约驱动的 5 层防错状态机、在制品并发控制与人类专属验收质量门禁。", 10.5, False, COLOR_TEXT)
            ...
            clear_and_add_runs(p, [
                ("【差异化壁垒】\n", 10.5, True, COLOR_PRIMARY),
                (barrier_text, 10.5, False, COLOR_TEXT)
            ], space_before=4, space_after=4)
        elif "目标用户一句话描述" in txt:
            clear_and_add_runs(p, [("使用千问办公/AI IDE 进行日常软件需求拆解、架构设计、编码测试与敏捷交付的技术主管、敏捷项目经理、全栈工程师及独立开发者。", 10.5, False, COLOR_TEXT)], space_before=3, space_after=4)
        elif "核心痛点" in txt:
            pain_text = (
            ...
            clear_and_add_runs(p, [(pain_text, 10.5, False, COLOR_TEXT)], space_before=3, space_after=6)
        elif "如在专家套件中不封装 MCP" in txt:
            clear_and_add_runs(p, [("注：本套件采用本地原生轻量 CLI 引擎驱动，无需额外封装外部独立 MCP 连接器。", 9.5, False, COLOR_MUTED)], space_before=2, space_after=4)

    # 2. Table 0: Persona
    t0 = doc.tables[0]
    set_cell(t0.rows[0].cells[0], "要素", bold=True, font_size_pt=9.5, bg_hex="F1F5F9", align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell(t0.rows[0].cells[1], "详细描述", bold=True, font_size_pt=9.5, bg_hex="F1F5F9", align=WD_ALIGN_PARAGRAPH.CENTER)
    persona_data = [
    ...
    for row_idx, (k, v) in enumerate(persona_data, start=1):
        set_cell(t0.rows[row_idx].cells[0], k, bold=True, font_size_pt=9.0, bg_hex="F8FAFC")
        set_cell(t0.rows[row_idx].cells[1], v, bold=False, font_size_pt=9.0)
    ...
    for ci, ht in enumerate(h_titles):
        set_cell(h_row.cells[ci], ht, bold=True, font_size_pt=9.5, bg_hex="F1F5F9", align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    for sc in scenes:
        ...
        for col_idx, val in enumerate(sc):
            set_cell(row.cells[col_idx], val, bold=(col_idx==0), font_size_pt=9.0, bg_hex="F8FAFC" if col_idx==0 else None)
    ...
    for ci, ht in enumerate(h_titles2):
        set_cell(h_row2.cells[ci], ht, bold=True, font_size_pt=9.5, bg_hex="F1F5F9", align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    for ci, ht in enumerate(h_titles3):
        set_cell(h_row3.cells[ci], ht, bold=True, font_size_pt=9.5, bg_hex="F1F5F9", align=WD_ALIGN_PARAGRAPH.CENTER)

    mcp_row = t3.add_row()
    set_cell(mcp_row.cells[0], "多专家协同原生引擎\n(无外部独立 MCP)", bold=True, font_size_pt=9.0, bg_hex="F8FAFC")
    set_cell(mcp_row.cells[1], "本套件采用本地轻量 CLI 脚本引擎与 Git 钩子驱动，无需依赖外部独立 MCP 服务，零额外网络开销，支持完全离线与私有化安全运行。", font_size_pt=9.0)
    set_cell(mcp_row.cells[2], "内置 35 项流转、门禁、度量与看板核心脚本工具（含 transition_task, verify_git_gate, check_stage_gate 等）", font_size_pt=9.0)

    # Save
    doc.save(out_path)
    doc.save(download_copy)
    print(f"Unified Proposal saved to {out_path} and {download_copy}")

if __name__ == "__main__":
    generate_unified_proposal()
