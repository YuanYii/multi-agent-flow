...
def resolve_report_dir(report_type: str) -> str:
    """报告归档目录：config paths.* 映射（锚定 data_root），默认 docs/04-研发过程/报告/{type}。
    ...
    return os.path.join(data_root, "docs", "04-研发过程", "报告", report_type)
...
def main(args: list = None):
    ...
    parser.add_argument("--output", default="", help="输出报告文件路径 (未指定时自动归档至 docs/04-研发过程/报告/{type}/)")
